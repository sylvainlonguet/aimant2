...
export default function Home({ data }) {
  const { node: { name: catComediennes } = {} } =
    data.allWpCategory.edges.find((e) => e.node.slug === COMEDIENNES) || {};
  const { node: { name: catComediens } = {} } =
    data.allWpCategory.edges.find((e) => e.node.slug === COMEDIENS) || {};
  const { node: { name: catMes } = {} } =
    data.allWpCategory.edges.find((e) => e.node.slug === METTEURS) || {};
  const { node: { name: catAuteurs } = {} } =
    data.allWpCategory.edges.find((e) => e.node.slug === AUTEURS) || {};
      ...
      <div id="contact" className="home__footer">
        ...
        {adresse.map((c) => (
          <a
            ...
            href={`mailto:${c.content.replace(/<\/?[^>]*?>/gi, "")}`}
          >
            <p style="font-size:10px;" className="infos">{c.content.replace(/<\/?[^>]*?>/gi, "")}</p>
          </a>
        ))}
